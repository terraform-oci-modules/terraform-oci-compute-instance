...
> inline block on `oci_core_instance`, so this module wires the same `secondary_network_interface`
> variable to a separate `oci_core_vnic_attachment` resource, attached immediately after the
> instance is created rather than atomically at launch. Functionally equivalent for almost all use
> cases; the only observable difference is a brief window where the instance exists before the
...
>
> **`force_destroy`**: AWS v6.4.0 added this as an escape hatch to destroy an instance even when
> `disable_api_termination`/`disable_api_stop` is set. Still N/A for OCI - there is no instance
> protection flag to override in the first place.
...
| `secondary_network_interface`                | `secondary_network_interface`              | Attach after launch via `oci_core_vnic_attachment`, not inline at launch |
...
| `disable_api_termination` / `disable_api_stop` / `force_destroy`                                                                                                                                                          | No instance protection in OCI                                                      |
...
| `placement_group` / `placement_group_id` / `placement_partition_number`                                                                                                                                                    | OCI Cluster Networks are out of scope                                              |
...
> **Not tracked here**: `putin_khuylo` is a non-technical boolean (`default = true`) that Anton
> Babenko includes across all `terraform-aws-modules` repos; it carries no infrastructure behavior
> and is intentionally excluded from this parity mapping.
...
| `secondary-network-interface` | Instance with a secondary VNIC attached post-launch into a second subnet, via `secondary_network_interface`                                                                             |
...
| `secondary-network-interface` - dedicated example | AWS covers `secondary_network_interface` inline in `complete`; OCI warrants standalone since it needs a second subnet and demonstrates the post-launch attachment resource |
...
Windows credentials, user data, metadata options (IMDSv2), secondary network interfaces.
